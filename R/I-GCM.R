...
#' @param num_workers number of threads used for feature selection, default num_workers=10
...
#' 
#' @return res a list stored all related results, which includes candidate_features,resid_mat and all_candidate_causal_set; 
...
InvariantGCM <- function(X_assoc, Y, envir, change_alpha=0.05, change_gcm=0, num_workers=10, verbose=FALSE){
    ...
    for (i in initial_assoc_len:1){
        ...
        gcm_obj <- gcm_test(X=X,Y=Y_outcome,Z=Z,regr.par=list(max_depth=4,max_delta_step=6), regr.method = "xgboost")
    ...
    for (j in 1:(nrow(res_mat)-1)){
        ...
        if (abs(gcm_stat_change)>0){
            ...
            if (verbose){
                cat("The stats change between the first",nrow(res_mat)-j+1,"and",nrow(res_mat)-j,"variables is",gcm_stat_change,"with a significance of",gcm_stat_change_P,"\n")
            }
            ...
            cat("The stats change between the first",nrow(res_mat)-j+1,"and",nrow(res_mat)-j,"variables is",gcm_stat_change,"please check your data.\n")
        }
    }
    res <- list(candidate_features=X_assoc,
                resid_mat = resid_mat,
                all_candidate_causal_set = causal_set_index)
    return(res)
}
